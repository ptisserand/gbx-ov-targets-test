...
inherit xov mercurial git prebuilt debuginfo versionator
...
function target-set_profile {
	into /
	dosym portage/${MYPROFILE}/${ARCH/\~} /etc/make.profile
...
function target-set_binhost {
	...
	echo "PORTAGE_BINHOST=${binhost_root}/${CATEGORY}/${PN}/${ARCH/\~}" >> ${D}/etc/portage/${PN}/${ARCH/\~}/make.defaults
}
...
function target_src_install() {
	...
	if [ -d "${S}/cache-config" ]; then
		insinto /etc/portage/
		doins -r "${S}/cache-config"
	fi
