...
DEPEND="dev-util/mercurial net-misc/rsync virtual/ssh"
...
function mercurial__is_cache {
	if [ -d "$1/.hg" ]; then
		if [ -n "$2" ]; then
			cache_repo=$($EHG_SHOW_CMD -R "$1" "paths.default")
			[ "$cache_repo" != "$2" ] && return -1
		fi
		return 0
	else
		return -1
...
function mercurial_fetch {
	...
	if ! mercurial__is_cache "$1" "$repo"; then
		# first check out
...
function mercurial_version {
	EHG_REVISION=$(${EHG_ID_CMD} -i -R "${1}")
	einfo $(${EHG_LOG_CMD} -R "${1}" --template \
		"Repository updated to revision {node|short}({rev})" -r \
		"${EHG_REVISION}")
}

function mercurial_shortrev {
	echo $(${EHG_ID_CMD} -n -R "${S}")
}
